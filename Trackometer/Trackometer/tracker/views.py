...
def add_to_db3(request):
	with open('C:\\Users\\Indu Reddy\\Desktop\\Chart_Data.csv','rt') as f:
		data=list(csv.reader(f))
		dates=data[0][4:]
		for row in data[1:]:
			for i in range(len(dates)):
				temp_lesson=chart(school_name=row[1],city=row[1],school_name=row[2],fee_segment=row[3],board=row[4],total_teachers=row[5],total_students=row[6],date=dates[i],count=row[7+i])
				temp_lesson.save()
